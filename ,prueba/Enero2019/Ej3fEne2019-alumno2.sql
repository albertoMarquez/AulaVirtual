...
BEGIN
  ...
  FOR r_taquilla IN c_taquillas LOOP
    ...
    --ERROR: ACTUALIZA INCORRECTAMENTE LOS DATOS DE VENTAS
    UPDATE taquillas SET ventas=v_taquilla_ventas*2 WHERE taquilla=r_taquilla.taquilla;
  END LOOP;
  IF v_puerta_ventas=0 THEN RAISE e_sin_venta; END IF;
--ERROR: NO HAY TRATAMIENTO DE EXCEPCIONES
END;
/
